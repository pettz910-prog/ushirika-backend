...
public interface PeerPaymentRepository extends JpaRepository<PeerPayment, UUID> {
    ...
    boolean existsByMemberAndPurpose(User member, PeerPaymentPurpose purpose);

    /** Used at final membership approval to find the applicant's registration-fee claim. */
    Optional<PeerPayment> findFirstByMemberAndPurposeOrderByCreatedAtDesc(User member, PeerPaymentPurpose purpose);
}
